...
В API реализован механизм сессий.
При каждом запросе выставляется/обновляется кука ``sessionid`` с временем жизни.
Эту куку клиент должен посылать на сервер при каждом запросе(исключение методы аутентификации).
